...
# Code modified from the Scott Thomason code.
# See: http://www.cpan.org/modules/by-authors/id/S/SC/SCOTTHOM/rationale.txt

# Note: We got this code from http://industrial-linux.org/RSSLite/
# but as of 2002, this site no longer exists!
# http://216.239.33.100/search?q=cache:yUI0vWHDAJIC:industrial-linux.org/+Scott+Thomason+RSSLite&hl=en&ie=UTF-8
...
sub preprocess {
    ...
    ##
    $$cr =~ s/\r\n?/\n/g;
    $$cr =~ s/&(?!([a-zA-Z0-9]+|#\d+|#[xX][\da-fA-F]+);)/&amp;/g;
    $$cr =~ s/[^\s\d\w!@#\$%^&\*i\(\)\-\+=:;"'<>,\.\/\?\x80-\xFF]/ /g;
